...
**ImageClassifierBackend** 是一款基于大模型技术的智能图片分类后端服务，为客户端提供高精度、低成本的图片分类API。系统采用FastAPI异步框架，支持**9种内容分类**和**8大维度分类**（内容、城市、颜色、存储、格式、分辨率、方向、相似组），通过智能缓存机制显著降低大模型调用成本，是AI照片分类、图像识别、批量图片处理的理想解决方案。

### ✨ 核心特性

- 🤖 **AI智能分类** - 基于大模型技术，准确率高达90%以上，支持9大内容类别
- 🎯 **多维度分类** - 支持8大分类维度（内容、城市、颜色、存储、格式、分辨率、方向、相似组）
- 🔍 **二维码检测** - 集成pyzbar库，快速检测图片中的二维码，自动分类
...
- 📸 **照片分类管理** - 为照片管理应用提供AI分类API服务，支持9大内容分类
- 🎨 **多维度分类** - 支持内容、城市、颜色、存储、格式、分辨率、方向、相似组等8大维度分类
- 🔲 **二维码识别** - 快速检测和分类二维码图片，支持pyzbar和AI双重识别
- 🗂️ **批量图片处理** - 支持批量上传和分类，提高处理效率
- 💾 **智能相册整理** - 为相册应用提供智能分类能力，多角度管理照片
- 🔍 **图像内容识别** - 识别照片类型、场景、物体等，支持AI和本地算法混合
- 📱 **移动应用后端** - 为iOS/Android应用提供图片分类服务
- 🌐 **SaaS服务** - 为多租户提供图片分类API服务
- 🏢 **企业级应用** - 支持高并发、高可用的企业级图片处理需求
- 🎯 **个性化分类** - 支持按颜色、格式、分辨率等维度进行个性化照片管理
...
| 🔲 二维码 | 95%+ | 二维码图片（pyzbar检测+AI识别） | 二维码管理、快速识别 |
...
| **🏙️ 按城市分类** | 拍摄地点（城市） | 💻 本地算法 | 从照片EXIF信息提取GPS坐标，通过本地数据库匹配城市 |
| **🎨 按颜色分类** | 照片主色调（背景颜色） | 🤖 AI分类 | AI识别照片的主要颜色，按颜色主题分类 |
| **📁 按存储分类** | 存储位置（文件夹路径） | 💻 本地算法 | 从文件路径提取目录信息，按存储位置分类 |
| **🖼️ 按格式分类** | 文件格式（JPEG、PNG、HEIC等） | 💻 本地算法 | 从文件MIME类型或扩展名提取格式信息 |
...
| **内容分类类别** | **9大类** | 覆盖常见照片场景（含二维码） |
| **分类维度** | **8大维度** | 内容、城市、颜色、存储、格式、分辨率、方向、相似组 |
| **二维码检测** | **双重保障** | pyzbar快速检测 + AI智能识别 |
| **缓存命中率** | **60-80%** | 智能缓存大幅降低成本 |
| **API响应时间** | **<2秒** | 包含大模型推理时间 |
| **并发处理能力** | **100+ QPS** | FastAPI异步架构 |
| **成本节省** | **60-80%** | 通过缓存机制实现 |
| **支持图片格式** | **JPG/PNG/WebP/GIF/MPO/HEIC** | 主流格式全覆盖 |
| **最大图片大小** | **10MB** | 可配置 |
...
- **分层处理** - 二维码检测→截图检测→缓存查询→远程推理→本地降级
- **混合推理** - 大模型失败时自动降级到本地模型
- **多维度分类** - 支持内容、城市、颜色、存储、格式、分辨率、方向、相似组等8大维度
- **快速检测** - 二维码优先检测，快速识别，减少大模型调用
...
- screenshot（手机截图）
- idcard（证件照）
- **qrcode（二维码）** - 新增，支持pyzbar检测和AI识别双重保障
- other（其它）

**提示词特点：**
- 如果图片中包含二维码，无论是否还有其他内容，都会优先分类为 `qrcode`
- 支持自定义分类规则和类别定义
- 可配置背景颜色识别（10种颜色：橙色、蓝色、红色、绿色、紫色、粉色、黄色、灰色、黑色、白色）
...
- 🎯 **多维度分类指南**: https://www.xintuxiangce.top/guide-multidimensional-classification.html - 8大分类维度使用指南
