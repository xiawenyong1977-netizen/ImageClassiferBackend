...
### 🔐 微信认证与额度管理
10. `POST /api/v1/auth/wechat` - 微信授权登录（获取openid）
11. `GET /api/v1/user/credits` - 查询用户额度
...
2. 用户点击授权按钮
3. 跳转到微信授权页面
4. 用户授权后，微信会回调并带上code参数
5. 将code发送到本接口获取openid
...
| openid | string | 微信用户唯一标识（需保存到localStorage） |
...
async function handleWeChatCallback() {
  ...
  // 调用接口换取openid
  const response = await fetch(`${API_BASE_URL}/api/v1/auth/wechat`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
  ...
  if (result.success) {
    ...
    localStorage.setItem('wechat_nickname', result.nickname);
    localStorage.setItem('wechat_avatar', result.avatar_url);
    
    console.log('✅ 授权成功:', result.openid);
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| X-WeChat-OpenID | string | 是 | 微信openid |
...
1. **首次关注**：自动分配100张额度
2. **每次使用**：提交编辑任务时会扣除对应的图片数量
3. **额度消耗**：
   - 1张图片 = 1张额度
   - 任务提交后立即扣减（包括失败的图片）
4. **额度清零**：额度用完后需要重新关注公众号获取额度
